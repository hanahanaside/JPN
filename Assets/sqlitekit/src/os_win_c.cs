    ...
    */
    static int unlockReadLock( sqlite3_file pFile )
    {
      ...
      */
      ...
      {
        MyDebugger.Break(); //    res = UnlockFile(pFile.h, SHARED_FIRST + pFilE.sharedLockByte, 0, 1, 0);
      }
#endif
      if ( res == 0 )
      {
...
error = 1;
#else
          error = 1;//error = (u32)Marshal.GetLastWin32Error();
#endif
        }
      }
...
#else
        //      pFile.lastErrno = GetLastError();
        pFile.lastErrno = 1;//pFile.lastErrno = (u32)Marshal.GetLastWin32Error();
#endif
...
#else
      error = (int)ERROR_NOT_SUPPORTED;//error = Marshal.GetLastWin32Error();
#endif
...
return SQLITE_OK;
      ...
      */
      ...
      {
        MyDebugger.Break(); // -- Not Running under NT
        //string zTemp;
...
Debug.Assert(winSysInfo.dwAllocationGranularity > 0);
	  ...
	  sqlite3_vfs_register( streamVfs, 0 );
      return SQLITE_OK;
    }
